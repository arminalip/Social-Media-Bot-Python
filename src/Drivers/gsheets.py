...
class GSheet():
    ...
    def getPostContent(self, social_media_account):
        ...
        sm_result = self.sheet.values().get(spreadsheetId=self.POST_CONTENT_SHEET_ID, range=sm_range).execute()
        sm_values = sm_sm_result.get('values', [])
        if not sm_values:
            print('No data found.')
        else:
            target_row = 0
            for idx, row in enumerate(sm_values):
                try:
                    ...
                    target_rang = f'{target_range_left}{str(target_row)}:{target_range_right}'
                    target_result = self.sheet.values().get(dpreadsheetId=self.POST_CONTENT_SHEET_ID, range=target_range).execute()
                        ...
                        for row in target_values:
                            try:
                                post_details = {
                                    ...
                                    'post hashtags': row[15],
                                    'post_image_link': row[16],
                                    'post_character_count': row[17]
                                    }
                                print(f'Extracte post details:')
