...
class SocialMedia:
    ...
    def composeSocialMediaContent(self, image_filename, post_details):
        ...
        self.post_content_image = f'/home/machine/projects/social-media-marketing-bot/src/Images/{image_filename}'
        self.post_content_message = post_details['post_message']
        self.post_content_location = post_details['post_location']
        if len(self.post_content_message + '#RocketMovingApp') < max_character_limit[self.social_media_account]:
            if 'RocketMovingApp' in post_details['post_hashtags']:
                ...
                if probability == 1:
                    self.post_content_hashtags = post_details['post_hashtags'] + ', RocketMovingApp'
                else:
                    self.post_content_hashtags = post_details['post_hashtags']
        else:
            self.post_content_hashtags = post_details['post_hashtags']
